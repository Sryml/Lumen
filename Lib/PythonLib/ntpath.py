...
def relpath(target_path, base_path="."):
    ...
    base_abs = normpath(join(os.getcwd(), base_path))
    ...
    base_list = string.split(base_rest, os.sep)
    while "" in target_list:
        target_list.remove("")
    while "" in base_list:
        base_list.remove("")
    n = 0
    for i in range(min(len(target_list), len(base_list))):
        if normcase(target_list[i]) != normcase(base_list[i]):
            break
        n = n + 1

    rel_list = [os.pardir] * (len(base_list) - n) + target_list[n:]
    if not rel_list:
        return os.curdir
